...
class TestPipLine():
    ...
    def SaveDecodeFile(self):
        import datetime
        nowTime = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')  # çŽ°åœ¨
...
class SLTester(TestPipLine):
    ...
    def evaluation(self, batch, dataset, blocking=False):
        """
            :param G: the model
        ...
        snode_id = G.filter_nodes(lambda x: x.data['type']==NODE_TYPE['sentence'])
