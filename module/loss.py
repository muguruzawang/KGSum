...
class LossComputeBase(nn.Module):
    ...
    def sharded_compute_loss(self, batch, outputs, tgt,
                              shard_size,
                             normalization, extra_len):
        ...
        # src.shape = [4,3600]
        shard_state = self._make_shard_state(batch, outputs,tgt)
        for shard in shards(shard_state, shard_size):
            loss, stats = self._compute_loss(batch, extra_len, **shard)
...
class NMTLossCompute(LossComputeBase):
    ...
    def _make_shard_state(self, batch, output,tgt):
        return {
            "output": output,
            "target": tgt,
        }

    def _compute_loss(self, batch, extra_len, output, target):
        output = self._bottle(output)

        gtruth = target.view(-1)

        loss = self.criterion(output, gtruth,extra_len)
