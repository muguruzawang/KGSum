...
class MultiheadOnlyAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, dropout=0.1, use_final_linear=True):
        ...
        super(MultiheadOnlyAttention, self).__init__()
...
class MultiheadOnlyAttentionWithScore(nn.Module):
    """
    Multi-Head Attention module from
    "Attention is All You Need"
    :cite:`DBLP:journals/corr/VaswaniSPUJGKP17`.
    ...
    def __init__(self, head_count, model_dim, dropout=0.1, use_final_linear=True):
        ...
        super(MultiheadOnlyAttentionWithScore, self).__init__()
        self.head_count = head_count
    ...
    def forward(self, key, value, ent_score, query, mask=None,
                layer_cache=None, type=None):
        """
        Compute the context vector and the attention vectors.
        ...
        query = query / math.sqrt(dim_per_head)
        scores = torch.matmul(query, key.transpose(2, 3))
        batch_size, head_num, query_len, src_len = scores.shape
        ent_score = ent_score.unsqueeze(1).unsqueeze(2).expand(batch_size,head_num,query_len,src_len)
